...
Here single cell data is loaded and stored as a matrix. Also Labels are stored at this point
...
cell_info_list <- list()
cell_info_list[[1]] <- collect_cell_info(imzmlPath="U:/James/HMDBE_KEGG_10/230615_SIMA9_lyo_20_1_FINAL.imzML",path_spotlist = "U:/James/spotlists/230615_SIMA9_lyo_20_1_spotlist.txt")
cell_info_list[[2]] <- collect_cell_info(imzmlPath="U:/James/HMDBE_KEGG_10/230616_SIMA9_lyo_20_1_FINAL.imzML",path_spotlist = "U:/James/spotlists/230616_SIMA9_lyo_20_1_spotlist.txt")
cell_info_list[[3]] <- collect_cell_info(imzmlPath="U:/James/HMDBE_KEGG_10/230618_SIMA9_lyo_20_1_FINAL.imzML",path_spotlist = "U:/James/spotlists/230618_SIMA9_lyo_20_1_spotlist.txt")
...
Here the actual Monte Carlo Consensus Clustering is applied to single cell data. It is first clustered on mz 129.02 (itaconate) and the on mz 124.01 (taurine).
...
m3c_clustering <- function(mz_input=129.02){
  ...
  for(indic_m3 in indices){
    ...
    m3c_clusters_hc <- M3C::M3C(filtered_data$filtered_data, cores=1, iters = 3, maxK=8,seed = 42, des = desx_tmp,repsref = 20,repsreal = 20,clusteralg = "hc")
...
colz <- colorRampPalette(c("darkblue","orange","darkgreen","brown"))(length(indices))
...
## Volcano Plot of the Results of M3C against unprocessed M3C



```{r}
idx_A <- which(grepl("Ita-Taur\\+", cell_lab_comb))
idx_B <- which(grepl("Ita\\+Taur-", cell_lab_comb))

idx_A <- which(grepl("VEH", cell_lab))
idx_B <- which(grepl("LPS", cell_lab))


ms_mat_cardinal <- cells_all_mz_mat 
...
volc_plot_itataur <- volc_plot(labs=cell_lab_comb, "Ita-Taur\\+","Ita\\+Taur-", ms_mat_cardinal=cells_all_mz_mat,color_neg = "green", color_pos = "magenta", color_neutral = "gray")
volc_plot_native <- volc_plot(labs=cell_lab, "VEH","LPS", ms_mat_cardinal=cells_all_mz_mat,color_neg = "darkblue", color_pos = "brown", color_neutral = "gray")


plot(volc_plot_itataur$volc_plot)
plot(volc_plot_native$volc_plot)
```

## Loading Slice Culture Data
...
## Volcano Plot of slice cultures
...
volc_plot_SC <- volc_plot(labs=simple_labels_SC_all[c(idx_A,idx_B)], "VEH","LPS", ms_mat_cardinal=ms_mat_normed_SC_all[c(idx_A,idx_B),],color_neg = "darkblue", color_pos = "brown", color_neutral = "gray",samp_ratio=0.1)

plot(volc_plot_SC$volc_plot)
